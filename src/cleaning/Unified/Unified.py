...
class Unified():
    def __init__(self, csv_d, csv_c, fd_path, theta=0.01, beta=0.4):
        ...
        # === 新增：清除脏数据DataFrame中可能混入的制表符，并strip首尾空白 ===
        self.dirty_csv.replace(r'\t', '', regex=True, inplace=True)        # ← 新增
        self.dirty_csv = self.dirty_csv.applymap(lambda x: x.strip() if isinstance(x, str) else x)  # ← 新增
        ...
        # === 新增：清除干净数据DataFrame中可能混入的制表符，并strip首尾空白 ===
        self.clean_csv.replace(r'\t', '', regex=True, inplace=True)        # ← 新增
        self.clean_csv = self.clean_csv.applymap(lambda x: x.strip() if isinstance(x, str) else x)  # ← 新增
    ...
    def get_best_repair(self, devi_value, core_values):
        ...
        for core_sig in core_values:
            # 如果想防止下标越界，可以在这里加一行判断:
            if len(core_sig) != len(devi_value):
                continue
            cost = 0
            change_idx = []
            ...
            if cost < min_val:
                min_val = cost
                best_core = core_sig
        return min_val, best_core, cands
