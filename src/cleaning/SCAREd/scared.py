...
def check_string(string: str):
    """
    如果在 string 中匹配到 -inner_error-, -outer_error-, -inner_outer_error-, -dirty-original_error-，
    就拼接特定后缀；否则直接返回原始的 string。
    """
    if re.search(r"-inner_error-", string):
        ...
        # 注意此处原逻辑取了 string[-9:-4]，可视需要自行调整
        return "-original_error-" + string[-9:-4]
    else:
        # 没有匹配到上述任何模式时，直接返回原始字符串
        return string
...
class SCAREd:
    ...
    def evaluation(self):
        ...
        if True:
            if not PERFECTED:
                det_right = 0
                out_path = "/root/AutoMLClustering/src/cleaning/Exp_result/scared/" + task_name + "/onlyED_" + task_name + ".txt"
                f = open(out_path, 'w')
                sys.stdout = f
                end_time = time.time()
                ...
                out_path = "/root/AutoMLClustering/src/cleaning/Exp_result/scared/" + task_name + "/oriED+EC_" + task_name + ".txt"
                res_path = "/root/AutoMLClustering/src/cleaning/Repaired_res/scared/" + task_name + "/repaired_" + task_name + ".csv"
                ...
                out_path = "/root/AutoMLClustering/src/cleaning/Exp_result/scared/" + task_name + "/perfectED+EC_" + task_name + ".txt"
                res_path = "/root/AutoMLClustering/src/cleaning/Repaired_res/scared/" + task_name + "/perfect_repaired_" + task_name + ".csv"
                self.rep_csv.to_csv(res_path, index=False, columns=list(self.rep_csv.columns))
                f = open(out_path, 'w')
                sys.stdout = f
                end_time = time.time()
...
if __name__ == "__main__": 
    ...
    try:
        ...
        if not PERFECTED:
            stra_path = "./data_with_rules/" + task_name + "/noise/raha-baran-results-" + 'scared'+task_name
            if os.path.exists(stra_path):
                shutil.rmtree(stra_path)
            stra_path = "./DATASET/data_with_rules/" + task_name + "/noise/raha-baran-results-" + 'scared' + task_name
            if os.path.exists(stra_path):
                shutil.rmtree(stra_path)
            stra_path = "./data_with_rules/tax/split_data/raha-baran-results-" + 'scared' + task_name
            if os.path.exists(stra_path):
                shutil.rmtree(stra_path)
            stra_path = "./data_with_rules/tax/split_data/raha-baran-results-" + 'scared' + task_name
            if os.path.exists(stra_path):
