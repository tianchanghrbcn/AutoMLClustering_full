...
\begin{enumerate}[leftmargin=2.5em]
  \item \textbf{得分评估实验}（\S\ref{sec:score_eval_exp}）  
        ...
        平均分、方差等维度的全局表现；
  \item \textbf{错误率梯度实验}（\S\ref{sec:err_grad_exp}）  
        —— 将 \(r_{\text{tot}}\) 每5\%分为一段，观察各组合的得分与运行耗时随错误率递增的折线趋势；
  \item \textbf{错误类型对比实验}（\S\ref{sec:err_type_exp}）  
        —— 在固定 \(r_{\text{tot}}\) 的前提下，改变
        \(\bigl(r_{\text{miss}},r_{\text{anom}}\bigr)\) 二元分布，
        比较三大错误类型（缺失、格式/语义错误、离群噪声）
        被清洗程度与聚类指标之间的耦合关系。
\end{enumerate}
